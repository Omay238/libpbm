...
fn main() {
    ...
    let s = 512;
    let mut big_ppm = PPMFile::new(s, s, [255, 255, 255]);
    for x in 0..s {
        for y in 0..s {
            ...
            let m = value - chroma;
            let color = [((color_1[0] + m) * 255.0) as u8, ((color_1[1] + m) * 255.0) as u8, ((color_1[2] + m) * 255.0) as u8];

            big_ppm.set_pixel(x, y, color);
        }
    }
    big_ppm.save_ascii("big.ppm", Some("wow, this is a big image!")).unwrap();
}
